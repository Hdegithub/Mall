...
public class MallOrderController {
    @Autowired
    private ShoppingCartItemService newBeeMallShoppingCartService;
    @Autowired
    ...
    public Result<String> saveOrder(@ApiParam(value = "订单参数") @RequestBody SaveOrderParam saveOrderParam, HttpServletRequest request) {
        ...
        List<ShoppingCartItemVO> itemsForSave = newBeeMallShoppingCartService.getCartItemsForSettle(Arrays.asList(saveOrderParam.getCartItemIds()), userId);
        if (CollectionUtils.isEmpty(itemsForSave)) {
            ...
            UserAddress address = newBeeMallUserAddressService.getMallUserAddressById(saveOrderParam.getAddressId());
